...
private:
  ...
  static std::string format(const std::string& message, T code, const std::error_category& category) {
    const auto name = std::string(category.name());
    const auto text = category.message(static_cast<int>(code));
    if constexpr (std::is_unsigned_v<T>) {
      return "[" + name + ':' + std::to_string(static_cast<std::uintptr_t>(code)) + "] " + message + ": " + text;
    } else {
      return "[" + name + ':' + std::to_string(static_cast<std::intptr_t>(code)) + "] " + message + ": " + text;
    }
  }
};
