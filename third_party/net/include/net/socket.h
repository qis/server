...
public:
  ...
  net::task<bool> handshake() noexcept;

  // Reads data from socket.
  // Returns empty string_view on closed connection.
  net::task<std::string_view> recv(char* data, std::size_t size) noexcept {
    return tls_ ? tls_recv(data, size) : native_recv(data, size);
  }

  // Writes data to the socket.
  // Returns false on closed connection.
  net::task<bool> send(std::string_view data) noexcept {
...
private:
  ...
  net::task<std::string_view> tls_recv(char* data, std::size_t size) noexcept;
  net::task<bool> tls_send(std::string_view data) noexcept;

  net::task<std::string_view> native_recv(char* data, std::size_t size) noexcept;
  net::task<bool> native_send(std::string_view data) noexcept;
